...
def cmidd(x, y, z, h, base=2):
    """ Discrete mutual information estimator
        Given a list of samples which can be any hashable object
    """
    assert len(x) == len(y) == len(z) == len(h), "Arrays should have same length"
    xzh = np.c_[x, z, h]
    yzh = np.c_[y, z, h]
    xyzh = np.c_[x, y, z, h]
    return entropyd(xzh, base) + entropyd(yzh, base) - entropyd(xyzh, base) - entropyd(z, base) - entropyd(h, base)
    # xz = np.c_[x, z]
    # yz = np.c_[y, z]
    # xyz = np.c_[x, y, z]
    # return entropyd(xz, base) + entropyd(yz, base) - entropyd(xyz, base) - entropyd(z, base)
